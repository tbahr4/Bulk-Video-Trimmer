...
class VideoPlayer(tk.Frame):
    def __init__(self, parent, screenWidth: int, screenHeight: int, playOnOpen: bool, backgroundHeight: int):
        ...
        self.bFullscreen = FullscreenButton(self, root=root, size=buttonSize, widgetList=[self.canvas])

        # display elements
       # self.background.place(x=0, y=0)
        #self.canvas.place(x=0,y=0)
        self.progressBar.place(x=0, y=screenHeight, width=screenWidth, height=progressBarHeight)
...
class FullscreenButton(tk.Frame):
    ...
    """
    def __init__(self, parent, root, size: int, widgetList: list):
        super().__init__(parent)
        self.root = root
        self.parent = parent
    ...
    def toggleFullscreen(self):
        ...
        scaleX = maxX / WINDOW_WIDTH
        scaleY = maxY / WINDOW_HEIGHT
